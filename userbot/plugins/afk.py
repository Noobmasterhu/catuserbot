...
async def set_not_afk(event):
    ...
    if (("afk" not in current_message) or ("#afk" not in current_message)) and (
        ...
        shite = await event.client.send_message(
            event.chat_id,
            "`KAKASHI senpai is Back online! No Longer afk.\nWas afk for " + endtime + "`",
        )
        AFK_.USERAFK_ON = {}
        AFK_.afk_time = None
...
async def on_afk(event):  # sourcery no-metrics
    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "media":
            if AFK_.reason:
                message_to_reply = (
                    f"`KAKASHI-SAMA IS AFK .\n\nAFK Since {endtime}\nReason : {AFK_.reason}`"
                )
            else:
                message_to_reply = f"`KAKASHI-SAMA IS AFK .\n\nAFK Since {endtime}\nReason : SENPAI was in hurry..!!`"
            if event.chat_id:
...
async def _(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event, f"`SENPAI is Going afk! because ~` {AFK_.reason}", 5
            )
        else:
            await edit_delete(event, "`SENPAI is Going afk! `", 5)
        if BOTLOG:
            if AFK_.reason:
                await event.client.send_message(
                    BOTLOG_CHATID,
